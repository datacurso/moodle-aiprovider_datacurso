 ...
 *
 * The plugin keeps a local table {aiprovider_datacurso_consumption} storing per-user
 * consumption (user, service, action, credits, date). The privacy provider must declare it and
 * export/delete it with the rest of the user's personal data -- and export ALL of a user's rows,
 * not just the last one (export_data() writes to a path derived from the subcontext, so calling
 * it once per record makes each row overwrite the previous one).
 *
...
final class privacy_consumption_test extends provider_testcase {
     ...
     *
     * A single export_data() call per record would overwrite the previous one, leaving only the
     * last row in the export, so all rows must be exported together.
     */
