 ...
 *
 * Enforcement no longer happens in Moodle: the configured limit is forwarded to the
 * Datacurso Python service (token-manager), which enforces it centrally against the
 * user's real credit consumption within the configured window. This class only reads
 * the admin configuration and builds the headers carrying it to the service.
 *
...
class ratelimiter {
     ...
     */
    public function is_rate_limit_enabled(string $serviceid): bool {
     ...
     * @return int
     */
    public function get_service_limit(string $serviceid): int {
     ...
     * @return int
     */
    public function get_window_length_in_seconds(string $serviceid): int {
